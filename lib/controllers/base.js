...
function BaseController($vern) {
  ...
  $scope.publicRoute = '';
  $scope.enablePublicPost = false;
  $scope.enablePublicDelete = false;
  $scope.enablePublicPut = false;
  $scope.version = '1.0.0';
  $scope.$vern = $vern;
  ...
  $scope.init = function(configOptions) {
    if(configOptions) {
      ...
      $scope.enablePublicPost = configOptions.enablePublicPost ? configOptions.enablePublicPost : $scope.enablePublicPost;
      $scope.publicDeleteRoute = configOptions.enablePublicDelete ? configOptions.enablePublicDelete : $scope.enablePublicDelete;
  ...
  $scope.setCRUDRoutes = function() {
    ...
    if($scope.enablePublicPut.length > 0) {
      $scope.addRoute({
        method: 'put',
        path: $scope.publicRoute + '/:_id',
    ...
    if($scope.enablePublicPost.length > 0) {
      $scope.addRoute({
        method: 'post',
        path: $scope.publicRoute,
    ...
    if($scope.enablePublicDelete.length > 0) {
      $scope.addRoute({
        method: 'del',
        path: $scope.publicRoute,
  ...
  $scope.handleTotals = function(req, res, next) {
    ...
    $scope.beforeTotalCaller({req: req, $scope: $scope}, function(req, err) {
      ...
      if(Object.keys(search).length) {
        ...
        $vern.searchModule.convertSearchToConditions(search, conditions, searchOptions);
      }
  ...
  /**
   ...
   @param [req.params.stub] {Boolean} Send the stub for the model instead of the entire thing. *Default is false*
  ...
  $scope._get = function(req, res, next) {
    ...
    if(conditions._id) {
      if((conditions._id instanceof Array)) {
        for(var i = 0; i < conditions._id.length; i++) {
          try {
            conditions._id[i] = $vern.db.ID(conditions._id[i]);
          } catch(e) {
            return res.resp.handleError(400, 'Invalid ID');
          }
        }
      } else if(typeof conditions._id === 'object') {
        for(var i in conditions._id) {
          if((conditions._id[i] instanceof Array)) {
            for(var j = 0; j < conditions._id[i].length; j++) {
              try {
                conditions._id[i][j] = $vern.db.ID(conditions._id[i][j]);
              } catch(e) {
                return res.resp.handleError(400, 'Invalid ID');
              }
            }
        ...
        try {
          conditions._id = $vern.db.ID(conditions._id);
        } catch(e) {
          return res.resp.handleError(400, 'Invalid ID');
        }
      }
    ...
    $scope.beforeGetCaller({object: conf, req: args.req, res: args.res, $scope: $scope}, function(conf, err) {
      ...
      try {
        $vern.db.findWithRange(conf.db, conf.conditions, conf.fields, conf.skip, conf.limit, sort, function(err, rows) {
          ...
          var data_list = [];
          var out_method = 'output';
          if(conf.stub === true) {
            out_method = 'stub'
          }
          for(var i in rows) {
            var a = new $scope.model(rows[i]);
            data_list.push(a.strip_fields(conf.fields)[out_method](conf.is_admin));
          }
  ...
  $scope.afterTotalCaller = function(total, callback) {
    ...
    Q.all(promises).spread(function() {
      ...
      for(var i = 0; i < $scope.initStack['total']['after'].length; i++) {
        promises.push($scope.runStackLoop('total', 'after', i, total));
      }
      Q.all(promises).spread(function() {
  ...
  $scope.afterSumCaller = function(sum, callback) {
    ...
    Q.all(promises).spread(function() {
      ...
      for(var i = 0; i < $scope.initStack['sum']['after'].length; i++) {
        promises.push($scope.runStackLoop('sum', 'after', i, sum));
      }
      Q.all(promises).spread(function() {
        return callback(sum);
      }).fail(function(err) {
