...
function BaseController($vern) {
  ...
  $scope.addUsage = function(proc, type) {
    ...
    $scope[funcName] = function(data, callback) {
      var promises = [];
  ...
  $scope.handleTotals = function(req, res, next) {
    ...
    $scope.beforeTotalCaller({req: req, $scope: $scope}, function(req, err) {
      ...
      var searchType = req.params.searchType || '$or';
      if(Object.keys(search).length) {
        if (!conditions[searchType]) {
          conditions[searchType] = [];
        }

        for (var i in search) {
          var tempobj = {};
          tempobj[i] = new RegExp(search[i], req.params.searchOptions);
          conditions[searchType].push(tempobj);
        }
      }
  ...
  $scope._get = function(req, res, next) {
    ...
    if(req.params.not) {
      // TODO: fix this so we can pass natural mongodb params.
    ...
    new $scope.model().getById(req.params._id, function(err, obj) {
      ...
      $scope.beforeDeleteCaller({object: obj, req: req, res: res, $scope: $scope}, function(obj, err) {
        ...
        obj.del(function(err, obj) {
          ...
          $scope.afterDeleteCaller({ok: true, object: obj, req: req, res: res, $scope: $scope}, function(data, err) {
            if(err) {
              return res.resp.handleError(500, err);
            }
