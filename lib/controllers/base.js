...
function BaseController($parent) {
  ...
  $scope.init = function(configOptions) {
    if(configOptions) {
      $scope.modelName = (configOptions.model ? configOptions.model : $scope.model);
  ...
  /**
   ...
   @param {Function} controller The function which handles the request
   @param {Boolean} requiresAuth Whether the route requires authentication or not
   @param {Boolean} requiresAdmin A strict requirement for admin privileges, better suited to monitor privileges by role though.
   **/
  ...
  $scope.initRoutes = function() {
    for(var i in $scope.routes) {
      ...
      if(conf.requiresSession) {
        if(conf.requiresAuthenticatedSession) {
          $parent.app[conf.method](conf.path, checkAPI, checkAuthenticatedSession, conf.controller);
        } else {
          $parent.app[conf.method](conf.path, checkAPI, checkSession, conf.controller);
        }
      } else if(conf.requiresAccessToken) {
        if(conf.requiresAuth === true) {
          $parent.app[conf.method](conf.path, checkAPI, checkAuth, requiresAuth, conf.controller);
        } else {
          $parent.app[conf.method](conf.path, checkAPI, checkAuth, conf.controller);
        }
      } else {
        if(conf.requiresAuth === true) {
  ...
  $scope.check = function(fields, model, res, checkFields) {
    if(!checkFields) {
      if(!res) {
        res = model;
        model = null;
    ...
    if(!model.__validations) {
      return fields;
    }

    var iterateFields = model.__validations, v;

    for(var i in iterateFields) {
      var key = i;
      var optional = false;
      if(i.charAt(0) === '?') {
      ...
      if(!fields[key] && optional === true && (checkFields && checkFields.indexOf(key) <= -1)) {
        continue;
      }

      if(!fields[key] && (checkFields && checkFields.indexOf(key) <= -1)) {
        continue;
      }

      if(typeof model.__validations[i] === 'function') {
        v = model.__validations[i](fields[key]);
        if(typeof v === 'object' && v.constructor === Error) {
          var resp = new ResponseModel();
          resp.handleError(res, 500, v);
          return null;
        ...
        // it is a string
        v = model.__defaultValidations[model.__validations[i]](key, fields);
        if(typeof v === 'object' && v.constructor === Error) {
          var resp = new ResponseModel();
          resp.handleError(res, 500, v);
          return null;
  ...
  $scope.handleTotals = function(req, res, next) {
    var user = req.user;
    var resp = new ResponseModel(res);
    ...
    new $scope.model().count(req.params.conditions, function(err, total) {
      if(err) {
        console.log(err);
        return resp.handleError(res, 500, 'Error getting totals');
      }
      resp.data(total);
      resp.send();
    });
  ...
  $scope.handleSum = function(req, res, next) {
    ...
    if(!req.params.field) {
      return resp.handleError(res, 404, 'Missing field');
    }
    ...
    new $scope.model().sum(req.params.conditions, req.params.field, function(err, sum) {
      if(err) {
        console.log(err);
        return resp.handleError(res, 500, 'Error gettign sum');
      }
  ...
  $scope._get = function(req, res, next) {
    ...
    conditions = util.convertParamToObject(req.params.conditions);
    req.params.not = util.convertParamToObject(req.params.not);
    search = util.convertParamToObject(req.params.search);
    ...
    if(req.params._id) {
      try {
        conditions._id = global.db.ID(req.params._id);
      } catch(e) {
        return resp.handleError(res, 400, 'Invalid ID');
      }
    }
    ...
    if(req.params.not) {
      ...
      if(typeof req.params.not === 'object' && Object.keys(req.params.not).length > 0) {
        for(var i in req.params.not) {
          if(i === '_id') {
            try {
              req.params.not[i] = global.db.ID(req.params.not[i]);
            } catch(e) {
              return resp.handleError(res, 400, 'Invalid ID in not condition');
            }
          }
    ...
    if(modelObj.requiresOwner('list')) {
      if(!req.user && !req.session) {
        return resp.handleError(res, 403, 'You do not have access to this data');
      }
    ...
    $scope.listForTableData({
      ...
      if(err) {
        console.log(err);
        return resp.handleError(res, 400, err.error + ': ' + err.reason);
      }
  ...
  $scope.listForTableData = function(args, callback) {
    ...
    var conf = util.extend(defaults, args);
    ...
    try {
      global.db.findWithRange(conf.db, conf.conditions, conf.skip, conf.limit, sort, function(err, rows) {
        if(err)
  ...
  $scope._post = function(req, res, next) {
    ...
    if(params._id) {
      return new model().getById(params._id, function(err, eItem) {
        if(err || !eItem) {
          return resp.handleError(res, 404, 'Object not found');
        }
        // Add fields here.
        var modelObj = new model();
        if(modelObj.requiresOwner('post') && user.role !== 'admin') {
          if(eItem['_owner'] !== req.user._id.toString()) {
            return resp.handleError(res, 403, 'Object does not belong to you');
          }
        }
  ...
  $scope.saveAndUpload = function(eItem, req, res, next) {
    ...
    eItem.save(function(err, newModel) {
      if(err) {
        console.log(err);
        return resp.handleError(res, 500, 'An internal server error occurred');
      }
      ...
      resp.data(newModel.output(req.is_admin));
      resp.send(res);
    });
  };
  ...
  $scope._delete = function(id, req, res, next) {
    ...
    new $scope.model().getById(req.params._id, function(err, obj) {
      if(err || !obj === 0) {
        return resp.handleError(res, 404, 'Object does not exist');
      }
      ...
      if($scope.modelName === 'FileModel') {
        if(obj['diskLocation'].length > 0) {
          fs.unlink(obj['diskLocation'], function(e) {
            fs.rmdir(obj['diskFolder'], function(e) {});
          });
        };
      }
  ...
  $scope.handleOrder = function(req, res, next) {
    ...
    try {
      ...
      return resp.handleError(res, 400, 'Invalid JSON');
    }
    var q = [];
    for(var i = 0; i < objs.length; i++) {
      q.push({
        _id: global.db.ID(objs[i]._id)
      });
    }
    ...
    global.db.find(db, {
      ...
      if(err) {
        return resp.handleError(res, 500, 'An internal error occurred');
      }
  ...
  function checkAPI(req, res, next) {
    ...
    if(req.headers['access-token']) {
      new APIModel().getByAccessToken(req.headers['access-token'], function(err, access_token) {
        if(err) {
          console.log(err);
          return resp.handleError(res, 403, 'Authorization Required: and error occurred');
        }
        if(!access_token.length) {
          req.account = null;
          return next();
        }
        new UserModel().getById(access_token[0]._owner, function(err, account) {
          if(err || !account) {
            console.log(err);
            return resp.handleError(res, 404, 'Account owner not found');
          }
      ...
      return new ResponseModel().handleError(res, 500, 'Missing access token');
    }
  }

  function checkSession(req, res, next) {
    var resp = new ResponseModel();
    new APISessionModel().getById(req.headers['session-id'], function(err, session) {
      if(err) {
        console.log(err);
        return resp.handleError(res, 500, 'An error occurred getting the session');
      }
      console.log(session);
      if(!session) {
        return resp.handleError(res, 403, 'Authorization required: missing session');
      }

      if(session.access_token !== req.headers['access-token']) {
        return resp.handleError(res, 403, 'Authorization required: invalid session');
      }
  ...
  function checkAuthenticatedSession(req, res, next) {
    var resp = new ResponseModel();
    checkSession(req, res, function(req, res, next) {
      if(!req.session.user_id) {
        return resp.handleError(res, 403, 'Authorization required');
      }
      new UserModel().getById(req.session.user_id, function(err, user) {
        if(err || !user) {
          return resp.handleError(res, 404, 'User not found');
        }
  ...
  function checkAuth(req, res, next) {
    ...
    if(req.headers['authentication-key']) {
      ...
      global.db.find('users', {authenticationKey: req.headers['authentication-key']}, function(err, rows) {
        if(err) {
          console.log(err);
          return resp.handleError(res, 403, 'Authorization Required: an error occurred');
        }
        if(rows.length <= 0) {
          req.user = null;
          return next();
  ...
  function isAuthenticated(req, res, next) {
    ...
    if(!route) {
      ...
      return resp.handleError(res, 404, 'Route not found');
    }
