...
function BaseController($vern) {
  ...
  $scope.handleTotals = function(req, res, next) {
    ...
    $scope.beforeTotal({req: req}, function(req, err) {
      if(err) {
  ...
  $scope.handleSum = function(req, res, next) {
    ...
    $scope.beforeSum({req: req}, function(req, err) {
      if(err) {
        return res.resp.handleError(500, err);
      }
  ...
  $scope.checkAuth = function(req, res, next) {
  ...
  $scope.requiresAuth = function(req, res, next) {
    return $scope.isAuthenticated(req, res, next);
  }

  $scope.isAuthenticated = function(req, res, next) {
    if(!req.user) {
...
/* Data is an object {req}, must return req to the callback */
BaseController.prototype.beforeTotal = function(data, callback) {
  return callback(data.req);
};

/* Sends total, expects total back in callback */
BaseController.prototype.afterTotal = function(total, callback) {
  return callback(total);
};

/* Data is an object {req}, must return req to the callback */
BaseController.prototype.beforeSum = function(data, callback) {
  return callback(data.req);
};

/* Sends sum, expects sum back in callback */
BaseController.prototype.afterSum = function(sum, callback) {
  return callback(sum);
};
...
BaseController.prototype.parseRoute = function($vern, $scope, conf) {
  ...
  if(conf.requiresAuth === true) {
    $vern.app[conf.method](conf.path, $scope.checkAuth, $scope.requiresAuth, conf.controller);
  } else {
    $vern.app[conf.method](conf.path, $scope.checkAuth, conf.controller);
  }
};
