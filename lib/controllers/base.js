...
/* Data is an object {model, skip, limit, is_admin, sort, sortDir: 1, conditions}, must return data object to the callback */
BaseController.prototype.beforeGet = function(data, callback) {
  return callback(data.object);
};

/* Data is an array of objects */
BaseController.prototype.afterGet = function(data, callback) {
  return callback(data);
};

/* Data is an object {object: Model, req: Request}, must return object to the callback */
BaseController.prototype.beforePost = function(data, callback) {
  return callback(data.object);
};

/* Data is an object {object: Model, req: Request} */
BaseController.prototype.afterPost = function(data, callback) {
  return callback(data);
};

/* Data is an object {object: Model, req: Request}, must return object to the callback */
BaseController.prototype.beforeDelete = function(data, callback) {
  return callback(data.object);
};

/* Data is an object {ok: Boolean, object: Model} */
BaseController.prototype.afterDelete = function(data, callback) {
  return callback(data);
};
...
/**
 ...
 @method init
 **/
BaseController.prototype.init = function(configOptions) {
  var $scope = this;
  if(configOptions) {
 ...
 */
BaseController.prototype.e = function(message, data) {
 ...
 **/
BaseController.prototype.setCRUDRoutes = function() {
  var $scope = this;
...
BaseController.prototype.addRoute = function(opts) {
  ...
  var defaults = {
    method: 'get',
    path: '/',
    controller: function(req, res, next){ res.resp.send(); next(req, res) },
    requiresAuth: false,
    requiresAdmin: false
  };
...
BaseController.prototype.initRoutes = function() {
  ...
  for(var i in $scope.routes) {
    ...
    if(conf.requiresAuth === true) {
      $scope.$vern.app[conf.method](conf.path, $scope.checkAuth, $scope.requiresAuth, conf.controller);
    } else {
      $scope.$vern.app[conf.method](conf.path, $scope.checkAuth, conf.controller);
    }
  }
  return $scope;
};

BaseController.prototype.validate = function(data, res, model) {
  var $scope = this;
  if(!model) {
...
BaseController.prototype.handleList = function(req, res, next) {
  var $scope = this;
  $scope.get(req, res, next);
};

BaseController.prototype.handleListAccount = function(req, res, next) {
  var $scope = this;
  $scope.getAccount(req, res, next);
};

BaseController.prototype.handleListAdmin = function(req, res, next) {
  var $scope = this;
  $scope.getAdmin(req, res, next);
};

BaseController.prototype.handlePost = function(req, res, next) {
  var $scope = this;
  $scope.post(req, res);
};

BaseController.prototype.handleDelete = function(req, res, next) {
  var $scope = this;
  $scope.delete(req.params._id, req, res);
};

BaseController.prototype.handleTotals = function(req, res, next) {
  var $scope = this;
  var user = req.user;
...
BaseController.prototype.handleSum = function(req, res, next) {
  var $scope = this;
  var user = req.user;
 ...
 **/
BaseController.prototype._get = function(req, res, next) {
  var $scope = this;
  ...
  conditions = $scope.$vern.util.convertParamToObject(req.params.conditions);
  req.params.not = $scope.$vern.util.convertParamToObject(req.params.not);
  search = $scope.$vern.util.convertParamToObject(req.params.search);
  ...
  if(req.params._id) {
    try {
      conditions._id = $scope.$vern.db.ID(req.params._id);
    } catch(e) {
  ...
  if(req.params.not) {
    ...
    if(typeof req.params.not === 'object' && Object.keys(req.params.not).length > 0) {
      for(var i in req.params.not) {
        if(i === '_id') {
          try {
            req.params.not[i] = $scope.$vern.db.ID(req.params.not[i]);
          } catch(e) {
...
BaseController.prototype.getAccount = function(req, res, next) {
  var $scope = this;
  var user = req.user;
  req.params.conditions = $scope.$vern.util.convertParamToObject(req.params.conditions);
  req.params.conditions._owner = user._id.toString();
  $scope._get(req, res, next);
};
...
/**
 ...
 @param [next] {Function} A chainable function useful for situations which may need to call another function outside of this scope.
 **/
BaseController.prototype.getAdmin = function(req, res, next) {
  var $scope = this;
  req.admin = true;
  $scope._get(req, res, next);
};
 ...
 **/
BaseController.prototype.listForTableData = function(args, callback) {
  var $scope = this;
  var defaults = {
    model: undefined,
    skip: 0,
  ...
  $scope.beforeGet({object: conf, req: args.req, res: args.res}, function(conf, err) {
    if(err) {
    ...
    try {
      $scope.$vern.db.findWithRange(conf.db, conf.conditions, conf.skip, conf.limit, sort, function(err, rows) {
        if(err)
          return callback(err, null);
...
/**
 ...
 @param res {Socket} The response socket for Restify
 **/
BaseController.prototype.post = function(req, res, next) {
  var $scope = this;
  if(!res.checked) {
    var checks = {};
    var m = new $scope.model();
...
BaseController.prototype._post = function(req, res, next) {
  var $scope = this;
  var user = req.user;
...
BaseController.prototype.saveAndUpload = function(eItem, req, res, next) {
  var $scope = this;
  $scope.beforePost({object: eItem, req: req, res: res}, function(eItem, err) {
    if(err) {
...
BaseController.prototype._delete = function(id, req, res, next) {
  var $scope = this;
  var user = req.user;
  ...
  new $scope.model().getById(req.params._id, function(err, obj) {
    ...
    $scope.beforeDelete({object: obj, req: req, res: res}, function(obj, err) {
      if(err) {
        return res.resp.handleError(500, err);
      }
...
BaseController.prototype.handleOrder = function(req, res, next) {
  var $scope = this;
  var user = req.user;
  ...
  for(var i = 0; i < objs.length; i++) {
    q.push({
      _id: $scope.$vern.db.ID(objs[i]._id)
    });
  }
...
BaseController.prototype.getRoute = function(path, method) {
  var $scope = this;
...
BaseController.prototype.checkAuth = function(req, res, next) {
  var $scope = this;
  res.resp = new $scope.$vern.models.ResponseModel(res);
  ...
  if(req.headers['authentication-key']) {
    ...
    new $scope.$vern.models.UserModel().getByAuthenticationKey(req.headers['authentication-key'], function(err, rows) {
      if(err) {
        console.log(err);
      ...
      req.user = new $scope.$vern.models.UserModel(rows[0]);
      return next();
    });
  } else {
    return next();
...
BaseController.prototype.requiresAuth = function(req, res, next) {
  var $scope = this;
  return $scope.isAuthenticated(req, res, next);
};

BaseController.prototype.isAuthenticated = function(req, res, next) {
  var $scope = this;
  ...
  var route = $scope.getRoute(req.route.path, req.route.method);
  if(!route) {
