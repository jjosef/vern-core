...
function vern(config) {
  ...
  database($scope.config, function(db) {
    ...
    restify.CORS.credentials = typeof $scope.config.CORS_CREDENTIALS === 'boolean' ? $scope.config.CORS_CREDENTIALS : true;
    if($scope.config.CORS_ORIGIN) {
      for(var i = 0; i < $scope.config.CORS_ORIGIN.length; i++) {
        restify.CORS.origins.push($scope.config.CORS_ORIGIN[i]);
      }
    }
