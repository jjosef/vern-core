...
function vern(config) {
  ...
  database(config, function(db) {
     ...
     */
    if(config.env[process.env.NODE_ENV].static_port) {
      var publicServer = restify.createServer({
        name: config.name + ' public file server'
      });

      publicServer.listen(config.env[process.env.NODE_ENV].static_port, function() {
        console.log('%s listening at %s in %s mode', publicServer.name, publicServer.url, NODE_ENV);
      });

      publicServer.get(config.uploads_serve_dir + '.*', restify.serveStatic({
    ...
    $scope.app.listen(config.env[process.env.NODE_ENV].port, function() {
      ...
      $scope.models = {};
      $scope.models.UserModel = require('./models/UserModel');
