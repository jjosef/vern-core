    ...
    $scope.app.listen($scope.localConfig.port, function() {
      ...
      $scope.loadControllers = function(dir, recursive, prefix) {
        ...
        files.forEach(function(filename) {
          var isDir = fs.statSync(dir + '/' + filename).isDirectory();
          if(isDir && recursive) {
            $scope.readControllersDir(dir + '/' + filename, recursive, filename + '_');
          } else if(isDir) {
            return;
          }
          ...
          $scope.controllers[prefix + filename.slice(0, -3)] = require(dir + '/' + filename);
      ...
      $scope.loadModels = function(dir, recursive, prefix) {
        ...
        files.forEach(function(filename) {
          var isDir = fs.statSync(dir + '/' + filename).isDirectory();
          if(isDir && recursive) {
            $scope.loadModels(dir + '/' + filename, recursive, filename + '_');
          } else if(isDir) {
            return;
          }
          ...
          var model = require(dir + '/' + filename);
