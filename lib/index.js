...
function vern(config) {
  ...
  $scope.extend = require('node.extend');
  ...
  $scope.searchModule = require('./search');
    ...
    $scope.app.listen($scope.localConfig.port, function() {
      ...
      $scope.loadControllers = function(dir, recursive, prefix) {
        ...
        files.forEach(function(filename) {
          ...
          if(isDir && recursive) {
            $scope.loadControllers(realdir + '/' + filename, recursive, prefix + filename + '_');
          } else if(isDir) {
            return;
          }
      ...
      $scope.unloadRoutes = function() {
        for(var i in $scope.app.routes) {
          $scope.app.rm(i);
        }
      };
...
util.inherits(vern, events.EventEmitter);
