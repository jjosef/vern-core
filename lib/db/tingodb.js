/*
  ...
  setup local config for tingo driver:
  development: {
    // Database, TingoDB
    dbdriver: 'tingodb',
    databases: {
      tingodb: {
        db: __dirname + '/tingo-db'
      }
    }
  },
...
var tingoFunctions = function(db) {
  var $scope = this;
  $scope.locks = {};
  ...
  $scope.setRaw = function(collection, criteria, objNew, options, callback) {
    ...
    db.collection(collection).findAndModify(criteria, [['_id', 'asc']], objNew, options, callback);
  };
  ...
  $scope.bulkRemove = function(collection, data, options, callback) {
    ...
    var cObj = db.collection(collection);
    cObj.remove(data, options, function(err, removed) {
      if(err) {
        console.log(err);
      }
      callback(err, removed);
