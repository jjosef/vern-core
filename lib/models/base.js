...
BaseModel.prototype.__outputAdditions = [];
BaseModel.prototype.__outputRemovals = [];
BaseModel.prototype.output = function(admin, noPre) {
  ...
  var j;
  for(j = 0; j < this.__outputAdditions.length; j++) {
    ret[this.__outputAdditions[j]] = this[this.__outputAdditions[j]];
  }

  for(j = 0; j < this.__outputRemovals.length; j++) {
    if(typeof ret[this.__outputRemovals[j]] !== 'undefined') {
      delete ret[this.__outputRemovals[j]];
    }
  }
...
/**
 * Method to alter the default output data by adding a field to it.
 *
 * @method addOutput
 * @param key
 * @returns {Object}
 */
BaseModel.prototype.addOutput = function(key) {
  var $scope = this;
  if($scope.__outputAdditions.indexOf(key) > -1) {
    return;
  }
  $scope.__outputAdditions.push(key);
  return $scope;
};

/**
 * Method to alter the default output data by removing a field from it.
 *
 * @method removeOutput
 * @param key
 * @returns {Object}
 */
BaseModel.prototype.removeOutput = function(key) {
  var $scope = this;
  if($scope.__outputRemovals.indexOf(key) > -1) {
    return;
  }
  $scope.__outputRemovals.push(key);
  return $scope;
};
