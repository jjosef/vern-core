...
BaseModel.prototype.addCustomFunction = function(funcName, i) {
  ...
  $scope[funcName] = function(val, cb) {
    var data = {};
    data[i] = val;
    $scope.__$parent.db.find($scope.__type, data, function(err, res) {
      if(err) {
        return cb(err, null);
      }
      var d = [];
...
BaseModel.prototype.save = function(cb, callSaveAfter) {
  ...
  if($scope._id) {
    ...
    $scope.__$parent.db.count(this.__type, function(err, count) {
  ...
  $scope.__$parent.db.set(this.__type, data, function(err, doc) {
    if(err) {
      if(cb) {
        cb(err, doc);
      }
